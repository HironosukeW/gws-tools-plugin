...
1. 対象表でそのスプレッドシートが**書き込み可**であることを確かめる
2. **書き込む前に、同じ範囲を `values get` で読む。** 今そこに何が入っているかを確認する
3. **何をどう変えるのかをユーザーに提示し、承認を得る。** スプレッドシート名・範囲・変更前後の値を示す。範囲が広いときは件数と代表例を示す
4. 承認を得てから書く
5. 書いたあと、同じ範囲を読み直して結果を報告する
